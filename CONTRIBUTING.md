...
The `dev` partition provides root `devShells`, `formatter`, and `checks` using the selected `nixpkgs` input. The `cross-config-fmt` executable is available in the default shell. `dev/fixtures.nix` provides focused nix-unit suites and raw, lazy failure fixtures, accepting `nixpkgs` explicitly and defaulting to the root selection. Examples run through the test suites. Consumers may override `nixpkgs` or use `follows` while keeping one nixpkgs revision within each node collection. Compatibility selections stay outside member lockfiles and consumer dependency graphs.
