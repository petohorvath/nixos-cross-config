...
def main():
    ...
    parser.add_argument("--baseline", default="3fa01e7b5b92a707a857b9a85a13e8feae0dd89a")
    ...
    pins = json.loads(command(nix_command() + [
        "--expr",
        f"let f = builtins.getFlake {nix_string(repo / 'dev')}; in "
        "builtins.mapAttrs (_: input: { path = input.outPath; revision = input.rev; }) "
        "{ inherit (f.inputs) stable unstable; }",
    ]))
    system = command(["nix", "eval", "--impure", "--raw", "--expr", "builtins.currentSystem"]).strip()
    ...
    record = {
        ...
        "workload_sha256": {name: digest((repo / name).read_bytes()) for name in workload_files},
        "runner_sha256": digest(Path(__file__).read_bytes()),
        "nix_version": command(["nix", "--version"]).strip(),
        "gnu_time_version": command(["/usr/bin/time", "--version"]).splitlines()[0],
        "python_version": platform.python_version(),
        "platform": platform.platform(),
        "cpu_model": next(line.split(":", 1)[1].strip() for line in cpu_info.splitlines() if line.startswith("model name")),
            ...
            for pair in ["warmup"] + list(range(args.pairs)):
                ...
                for variant in variants:
                    expression = (
                        ...
                        + (f"count = {args.count};" if workload == "writable-tags" else f"system = {nix_string(system)};")
                        + " }"
                    )
                    label = f"{workload}-{pin}-{variant}-{pair}"
                    timing_path = output / f"{label}.time"
                    invocation = ["/usr/bin/time", "-f", "%e %U %S %M", "-o", str(timing_path)] + nix_command() + ["--expr", expression]
                    ...
                    if output_digest != expected:
                        raise RuntimeError(f"{label}: baseline/candidate outputs differ")
                    (output / f"{label}.json").write_text(result.stdout)
                    elapsed, user, kernel, rss = timing_path.read_text().split()
                    row = {
                        "workload": workload, "pin": pin, "variant": variant, "pair": pair,
                        "wall_seconds": float(elapsed), "process_wall_seconds": wall,
                        "user_seconds": float(user), "system_seconds": float(kernel),
                        "peak_rss_kib": int(rss), "output_sha256": output_digest,
                    }
                    record["warmups" if pair == "warmup" else "measurements"].append(row)
                print(f"{workload} {pin}: pair {pair} matched", flush=True)
            rows = [row for row in record["measurements"] if row["workload"] == workload and row["pin"] == pin]
...
def nix_command():
    return [
        ...
        "--json", "--impure", "--offline", "--option", "eval-cache", "false",
...
def validate(workload, value, count):
    if workload == "writable-tags":
        if len(value) != count or any(row["value"] != f"contributed-{index}" for index, row in enumerate(value)):
            raise RuntimeError("Unexpected stress-workload values")
        assertions = [row["assertions"] for row in value]
    else:
        if value["publication"] != {"domain": "application.example", "upstream": "http://192.0.2.10:8080"}:
            raise RuntimeError("Unexpected NixOS publication")
        if value["proxyPass"] != "http://192.0.2.10:8080" or 80 not in value["firewallPorts"]:
            raise RuntimeError("Unexpected NixOS nginx/firewall configuration")
        assertions = [value["assertions"]]
    if not all(flag is True for nodes in assertions for flags in nodes.values() for flag in flags):
...
def summarize(rows):
    ...
    for metric in ["wall_seconds", "peak_rss_kib"]:
        ...
        for variant in ["baseline", "candidate"]:
            ...
            result[metric][variant] = {"median": statistics.median(values), "min": min(values), "max": max(values)}
        result[metric]["ratio_of_medians"] = result[metric]["candidate"]["median"] / result[metric]["baseline"]["median"]
        pairs = sorted({row["pair"] for row in rows})
        changes = []
        for pair in pairs:
            values = {row["variant"]: row[metric] for row in rows if row["pair"] == pair}
            changes.append(100 * (values["candidate"] / values["baseline"] - 1))
        result[metric]["paired_change_percent"] = {"median": statistics.median(changes), "min": min(changes), "max": max(changes)}
    return result
...
def report(workload, record):
    dimensions = (
        ...
        if workload == "writable-tags" else
    ...
    for pin, summary in record["summaries"][workload].items():
        for metric, unit, scale in [("wall_seconds", "Wall (s)", 1), ("peak_rss_kib", "Peak RSS (MiB)", 1024)]:
            data = summary[metric]
            cells = []
            for variant in ["baseline", "candidate"]:
                sample = data[variant]
                cells.append(f"{sample['median']/scale:.3f} ({sample['min']/scale:.3f}–{sample['max']/scale:.3f})")
            paired = data["paired_change_percent"]
            lines.append(f"| {pin} | {unit} | {cells[0]} | {cells[1]} | {data['ratio_of_medians']:.3f} | {paired['median']:+.2f}% ({paired['min']:+.2f}%–{paired['max']:+.2f}%) |")
    lines += ["", "## Reproduction", "", "Run from the repository root with the locked inputs already available locally and no competing evaluator or build workloads. Choose a new output directory:", "", "```bash", f"python3 dev/benchmarks/run.py --baseline {record['baseline_revision']} --pairs {record['pairs']} --count {record['count']} --output /tmp/cross-config-benchmark", "```", ""]
    for pin, source in record["pins"].items():
        lines.append(f"- {pin}: `{source['revision']}`, resolved locally as `{source['path']}`.")
    lines += ["", "The runner writes both reports, raw measurements, timed commands' outputs, and frozen source/workload trees. There is no numerical performance gate; the stress cost of supported evaluation is accepted. These workloads measure evaluation only and do not predict deployment or full system-build costs.", ""]
    return "\n".join(lines)
