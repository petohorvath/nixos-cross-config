...
- Run named value comparisons, expected errors, native recursion cases, and full diagnostic assertions through one test collection, with execution details handled by the shared runner.
- **Breaking (development interface):** replace separate evaluation, diagnostic, and value-cycle check outputs with `checks.<system>.tests`. The development shell provides `cross-config-test` for complete or focused runs, including all assertions for each selected test. Rename the collection loader from `dev/fixtures.nix` to `dev/tests.nix` and remove its separate `tests` and `failures` trees. Rejection tests now require the intended error message as well as its type.
...
- **Breaking (development interface):** root `lib.tests`, `lib.failures`, and example `nixosConfigurations`. The shared runner loads suites from `dev/tests.nix`; raw scenarios become private fixtures and examples remain covered by the tests.
...
For focused tests, replace direct `nix-unit` invocations and raw `failures.<fixture>` evaluations with `cross-config-test <suite-or-test>` inside the development shell. For example, use `cross-config-test merging.testHosts` or `cross-config-test merging.testRejectsLocalConflict`. Use `cross-config-test --list` to find the current names. The runner checks the selected case's value or error and all required diagnostic fragments. `dev/tests.nix` is the collection loader; its definitions alone do not execute assertions. Root `nix flake check --no-update-lock-file --print-build-logs` remains the complete validation command. See [focused checks](docs/development.md#focused-checks) for exact-revision commands.
...
Flake consumers keep the same module and constructor exports. The library no longer exports example nodes; run `cross-config-test example` and `cross-config-test flakeModule.example` or construct nodes from the files in `examples/`.
