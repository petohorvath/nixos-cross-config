...
The policy runner verifies the resolved input, nonempty host checks, and unchanged member sources and lock, then records the full check result and exact project, checker, policy-record, and dependency revisions. Both approved revisions are required on `x86_64-linux` and `aarch64-linux`. Compliance and committed-default jobs remain separate; project-owned formatting and lint run through root checks. Pin approval, compatibility execution, and reporting belong to the policy implementation; the member caller stays small and stores no copy of the compatibility pair.
...
The caller uses `name: Policy` and owns the release selection and `required_architectures` settings. Policy v0.4.0 replaces central member selection and architecture records with this caller declaration; central records retain enrollment identities and approved pins. The shared workflow derives ordinary and compatibility jobs from the caller settings. Both Linux architectures remain required.

An ordinary upgrade proceeds through a reviewed member PR without a central activation change. Verify successful stable and unstable runs on both declared architectures and observed statuses matching required merge gates. Formatting and lint remain in project-owned root checks after removal of the policy's separate lint jobs. The [development instructions](../development.md#ci-and-policy) describe status derivation and the merge-gate transition. Live merge-setting changes require separate authorization; passing local checks does not configure gates or authorize a merge.
