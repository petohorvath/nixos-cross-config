...
Development composition passes the selected inputs explicitly to one test runner, shared by `checks.<system>.tests` and the shell's `cross-config-test` command. Suites, the runner, and fixture helpers stay under `tests/`; examples stay under `examples/` and run through those suites instead of root `nixosConfigurations`. Public `lib` contains the constructor, while `dev/tests.nix` loads the complete test collection with root defaults for direct inspection. Shells, formatters, checks, fixtures, and examples therefore share one selection per run. Check names omit stable/unstable labels. The [changelog](../../CHANGELOG.md#migration) records the interface migration.

The unified collection replaces the earlier `dev/fixtures.nix` interface with separate `tests` and `failures` attributes. That split exposed raw invalid expressions and left diagnostic and recursion coverage outside the named test collection. Each test now owns its expected value or error and any required trace fragments, so selecting a test runs all its assertions. The runner keeps offline inputs, memory-bounded evaluator processes, and trace capture private. Invalid configurations remain lazy private fixtures.
