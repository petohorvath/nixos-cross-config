...
[Tagged-destination tests](../tests/tagged-destinations.nix) cover writable tags, names, wrappers, defaults, priorities, receiver-local permissions, and laziness. Rejection cases declare their expected error type and message beside the expression, including contribution context and source locations. Reciprocal and tagged-destination suites also require native recursion errors for value-dependency cycles. nix-unit executes every assertion for a selected test; the [development guide](development.md#focused-checks) describes complete and focused runs.
