...
| F2 | The public factory uses a plain import in [flake.nix](../../flake.nix#L5), rather than the skill's `importApply` convention for functions returning modules. | Preserve the imported module's source attribution through an appropriate wrapper. Keep the public factory and zero-required-input contract intact. Validate attribution and behavior. | Tracked in [#11](https://github.com/petohorvath/nixos-cross-config/issues/11). |
...
Issues #11–#13 remain independent follow-ups. Issue #10 extends the existing diagnostic and cycle infrastructure only to cover writable tags; its legacy CLI and dependency conventions remain assigned to #12. Optional J1 remains deferred.
